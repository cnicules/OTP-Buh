<!-- Create an unsorted list of tram stops on each route represented in OSM map.
...
<xsl:transform version="1.0"
  ...
  <xsl:template match="/osm">
    <xsl:text>&#xA;</xsl:text>
    <route-stops>
      <xsl:for-each select="relation[tag[@k='route' and @v='tram']]">
        ...
        <route>
          ...
          <xsl:for-each select="member">
            ...
            <xsl:if test="@type='node'">
              <xsl:call-template name="extract-tram-stop-from-node">
  ...
  <xsl:template name="extract-tram-stop-from-node">
    <xsl:param name="nodeId"/>

    <!-- /osm/node[@id=$nodeId and tag[@k='railway' and
                                       (@v='tram_stop' or @v='station')]] -->
    <xsl:for-each
      select="key('node', $nodeId)[tag[@k='railway' and
                                       (@v='tram_stop' or @v='station')]]">
