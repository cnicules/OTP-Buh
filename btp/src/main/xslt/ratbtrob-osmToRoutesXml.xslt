<!-- Extract trolleybus route ids, names, and types from OSM route relations.
  Warn if trolleybus route relation in OSM data has no or empty "ref" tag.
  ...
  Param stopSeqsXml: ratbtrob-stopsequences.xml
    <stop-sequences>
...
<xsl:transform version="1.0"
  ...
  <xsl:template match="/">
    ...
    <routes>  
      ...
      <xsl:for-each select="osm/relation[tag[@k='route' and @v='trolleybus']]">
        ...
        <xsl:choose>
          <xsl:when test="'' = normalize-space($routeShortName)">
            <xsl:comment>
              <xsl:text>OSM trolleybus route id=</xsl:text>
          ...
          <xsl:otherwise>
            <route>
              ...
              <!-- route_type: 3=bus (future: 800=trolleybus) -->
              <xsl:attribute name="route_type">
                <xsl:text>3</xsl:text>
              </xsl:attribute>

              <!-- route_color: turquoise (not yet used by OTP) -->
              <xsl:attribute name="route_color">
                <xsl:text>327698</xsl:text>
              </xsl:attribute>
